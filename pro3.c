...
void modgen(int end, int blockframes, short* audioblock, int freq, int sr, int dur, double* env, int bd){
	...
	for(i=0; i < end; i+=blockframes){
		for(j=0; j < blockframes; j++, ndx++){
			...
			for(int h=1;h<200;h++){ //this is adding sinudoidal harmonics
				if(h*freq>=sr/2){ //nyquist frequency check
					break;
				}
				
				audioblock[j] += 16000*(1./h)*sin((ndx*twopi*h*freq)/sr);	
	  		}
	  		if(ndx>=dur*sr){ //this is ending the loop before it goes out of bounds
				continue;
			}
			else{
...
void addsyn(int dur){
	...
	prinf("How many harmonics would you like in the carrier?");
	scanf("%d",&harmnum);
...
//Function to generate the triangle wave.
//will generate up to 200 harmonics, or until
//it reaches the Nyquist frequency.
void triangle(int end, int blockframes, short* audioblock, int freq, int sr, int dur, double* env, int bd){
	...
	for(i=0; i < end; i+=blockframes){
		for(j=0; j < blockframes; j++, ndx++){
			...
			for(int h=1;h<200;h++){
				...
				audioblock[j] += 16000*((pow(-1,((n-1)/2)))/(n*n))*sin(ndx*twopi*n*freq/sr);	
  			}	
  			if(ndx>=dur*sr){
				continue;
			}
  	...
  	fwrite(audioblock,sizeof(bd), blockframes, fpout);
	}
...
//Function to generate the sawtooth wave.
//will generate up to 200 harmonics, or until
//it reaches the Nyquist frequency.
void modgen(int end, int blockframes, short* audioblock, int freq, int sr, int dur, double* env, int bd){
...
//menu output for shape selection
printf("Select wave shape:\n  1. Triangle \n  2. Square \n  3. Sawtooth\n");
scanf("%d", &shape);
...
switch (shape){
	case 1: 
			triangle(end, blockframes, audioblock, freq, sr, dur, env, depth);
			break;

	case 2: 
			square(end, blockframes, audioblock, freq, sr, dur, env, depth);
			break;

	case 3: 
			saw(end, blockframes, audioblock, freq, sr, dur, env, depth);
			break;
}
