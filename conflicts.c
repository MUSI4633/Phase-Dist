...
void modgen(int end, int blockframes, short* audioblock, int freq, int sr, int dur, double* env, int bd)
{
	...
	//pointList: position of jump points on wave
	//levelList: amplitude of jump points
	float* pointList, levelList; 
	breaks(pointList, levelList, jnum); 
	int spc = sr/freq; //samples per cycle		

	// For simplicity's sake, the levels in levelList
	// will not be destinations, but the amount subtracted
	// at that point. This will prevent upward jumps
	//that could send the amplitude of the wave over 1.										

	for(int i<jnum;i=0;i++){
		pointList[i] *= sps;
	}

	float slope = slope(&levelList);
...
//		short* audioblock, int sr, int dur)
{
	...
	for(int i < sizeof(&pointList); i = 0; i++)
	{
		...
		printf("Input jump amount as a percentage of amplitude (0.-1.):");
		scanf("%s", &levelList);
